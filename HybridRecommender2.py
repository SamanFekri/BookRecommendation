...
class HybridRecommender2(BaseItemSimilarityMatrixRecommender):

    RECOMMENDER_NAME = "Hybrid_Recommender2"
 ...
 #       slim_recommender = SLIM_BPR_Cython(self.URM_train, recompile_cython=False)
  #      slim_recommender.fit(epochs=50, batch_size=100, sgd_mode='sgd', learning_rate=1e-2, positive_threshold_BPR=1)
        ...
        best_parameters = {'topK': 220, 'shrink': 56, 'similarity':"jaccard", "feature_weighting": "TF-IDF"}
        ...
        self.W_sparse = 0.1 * rp3_recommender.W_sparse + 0.3 * cbf_recommender.W_sparse + 0.1*itemKNNCF.W_sparse + .7*P3alpha.W_sparse